...
#define SURFACE_PITCH_ALIGNMENT 256
    ...
    void *data)
{
    ...
    unsigned char *pCudaCopyMemTemp = NULL;
    unsigned int i;
    unsigned int j, copyPitchInBytes = 0;
            ...
            for (i=0; i<cudaEgl.planeCount; i++) {
                ...
                if (cudaEgl.frameType == CU_EGL_FRAME_TYPE_PITCH) {
                    ...
                    if (cudaEgl.planeCount==1) {
                        ...
                        if (CU_EGL_COLOR_FORMAT_YUV420_PLANAR == cudaEgl.eglColorFormat) {
                            bufferSize = cudaEgl.pitch * cudaEgl.height;
                            copyWidthInBytes = cudaEgl.pitch;
                            copyHeight = cudaEgl.height;
                            if (i>0) {
                        ...
                        } else if (CU_EGL_COLOR_FORMAT_YUV422_PLANAR == cudaEgl.eglColorFormat) {
                            bufferSize = cudaEgl.pitch * cudaEgl.height;
                            copyWidthInBytes = cudaEgl.width;
                            copyPitchInBytes = cudaEgl.pitch;
                            copyHeight = cudaEgl.height;
                            if (i>0) {
                                bufferSize >>= 1;
                                copyWidthInBytes >>= 1;
                            ...
                            /*
                             * Currently, CUeglFrame has pitch and width information only for the
                             * first plane. There are cases possible where pitch information
                             * is required for other planes. For e.g. in case of 1280x800, 422p
                             * , width of UV planes is 640. Due to hardware limitations, pl surface
                             * width is 256 byte aligned and hence in this case, pitch is 768.
                             * So, below code checks and modifies the pitch to nearest 256
                             * byte aligned value.
                             */
                            if(copyPitchInBytes % SURFACE_PITCH_ALIGNMENT) {
                                copyPitchInBytes = ((copyPitchInBytes / SURFACE_PITCH_ALIGNMENT)
                                                    + 1) * SURFACE_PITCH_ALIGNMENT;
                                bufferSize = copyPitchInBytes * copyHeight;
                            }
                        }
                    }
                } else {
                ...
                cuCtxPopCurrent(&oldContext);
                if (cuStatus != CUDA_SUCCESS) {
                    LOG_ERR("Cuda get resource failed with %d\n", cuStatus);
                    goto done;
                }
                if (i==0) {
                    pCudaCopyMem = (unsigned char *)malloc(bufferSize);
                    if (pCudaCopyMem == NULL) {
                        LOG_ERR("pCudaCopyMem malloc failed\n");
                        goto done;
                    }
                }
                ...
                if (cudaEgl.frameType == CU_EGL_FRAME_TYPE_PITCH) {
                    ...
                    if(cuStatus != CUDA_SUCCESS) {
                        LOG_ERR("cuda_consumer: pitch linear Memcpy failed, bufferSize =%d\n", bufferSize);
                        goto done;
                    }
                ...
                if (cuStatus == CUDA_SUCCESS) {
                    ...
                    if (cudaEgl.frameType == CU_EGL_FRAME_TYPE_PITCH) {
                        pCudaCopyMemTemp = pCudaCopyMem;
                        for (j = 0; j < copyHeight; j++) {
                            if (fwrite(pCudaCopyMemTemp, copyWidthInBytes, 1, ctx->outFile) != 1) {
                            ...
                            if (copyPitchInBytes)
                                pCudaCopyMemTemp = pCudaCopyMemTemp + copyPitchInBytes;
                            else
                                pCudaCopyMemTemp = pCudaCopyMemTemp + copyWidthInBytes;
                        }
                    } else {
