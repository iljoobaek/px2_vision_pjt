...
static int ConnectProducer(TestArgs *args)
{
    ...
    if (STANDALONE_CP_PRODUCER == args->standalone) {
...
static int ConnectConsumer(TestArgs *args)
{
    ...
    LOG_DBG("Server listening...\n");

    if (STANDALONE_CP_CONSUMER == args->standalone) {
        char msg[100];
        int readSize;

        /* To verify successfull connection establishment */
        readSize = recv(accept_sock , msg , 100 , 0);
        if (readSize > 0) {
            msg[readSize] = 0;
            LOG_DBG("Message received: %s\n", msg);
        } else {
            LOG_ERR("[C] Server didn't receive message!\n");
        }
    }
                                            ...
                                            TestArgs *args)
{
    ...
    } else if (STANDALONE_CP_CONSUMER == args->standalone) {
                                            ...
                                            TestArgs *args)
{
    ...
    if (args->standalone == STANDALONE_PRODUCER) {
        attrList[attrIndex++] = EGL_STREAM_CROSS_PROCESS_NV;
    } else if (args->standalone == STANDALONE_CP_PRODUCER) {
        attrList[attrIndex++] = EGL_STREAM_CROSS_PARTITION_NV;
    }
...
#ifndef NVMEDIA_GHSI
    if(args->standalone == STANDALONE_CP_CONSUMER) {
