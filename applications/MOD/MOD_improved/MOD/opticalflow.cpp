...
*/
//Front Camera-> Top-left image
Rect rect1(25,  60, 180, 220);
Rect rect2(205, 60, 230, 250);
Rect rect3(435, 75, 200, 210);

//Right Camera-> Top-right image
Rect rect4(675, 53, 170, 122);
Rect rect5(845, 35, 230, 295);
Rect rect6(1075, 75, 185, 155);

//Rear Camera-> Bottom-left image
Rect rect7(30, 445, 175, 195);
Rect rect8(205, 420, 230, 290);
Rect rect9(435, 425, 195, 230);

//Left Camera-> Bottom-right image
Rect rect10(665, 425, 180, 185);
Rect rect11(845, 390, 230, 310);
Rect rect12(1075, 410, 185, 150);

static int log_level = LOG_INFO;
#define oferr 35    //18
#define STATIC_NOISE_THOLD 0.30 //0.15
#define CENTER_ROI_MAG_THOLD 8  //3
#define CENTER_ROI_ANG_THOLD 15  //7
#define SIDE_ROI_MAG_THOLD 2.5 //0.75
#define SIDE_ROI_ANG_THOLD 45
...
bool isSideCam(int winnum)
{
    return (winnum == 4 || winnum == 6        //right cam, side ROI
        ||  winnum == 10 || winnum == 12);    //left cam, side ROI
...
bool isLeftROI(Point2f p1, Rect rect)
{
    return p1.x < (rect.x + rect.width / 2);
}
...
flow_index findMedianVector(Rect rect, Mat& prv, Mat& next, Mat& next_copy)
{
    ...
    int density = 50, bound = 20, count = 0;
...
float drawrect(Mat& prv, Mat& next, Mat& next_copy, Mat& imsparse,Rect rect,vector<Point2f> *points,vector<uchar> &status,vector<float> err,int winnum,bool perform_median_calc)
{
    ...
    {
        optFErr = oferr;
        flow_index median_flow_top, median_flow_bot;
        if(isSideCam(winnum))
            median_flow_top = findMedianVector(Rect(rect.x, rect.y, rect.width / 2, rect.height), prv, next, next_copy);
        else
            median_flow_top = findMedianVector(Rect(rect.x, rect.y, rect.width, rect.height / 2), prv, next, next_copy);
        ...
        if(isSideCam(winnum))
            median_flow_bot = findMedianVector(Rect(rect.x + rect.width / 2, rect.y, rect.width / 2, rect.height), prv, next, next_copy);
        else
            median_flow_bot = findMedianVector(Rect(rect.x, rect.y + rect.height / 2, rect.width, rect.height / 2), prv, next, next_copy);
    ...
    if(perform_median_calc || (winnum == 2 && median_mag_top > 0.25))
    {
        for(size_t y=0; y<points[0].size(); y++) 
        {
            ...
            //if(isInsideWindow(p1, rect) && isInsideWindow(p2, rect) && (status[y]))
            if(isInsideWindow(p1, rect) && (status[y]))
            {
                if(winnum == 4 || winnum == 5 || winnum == 6)
                    num_total++;
                if(err[y] < optFErr)
                {
                    ...
                    if(abs(point_mag) < STATIC_NOISE_THOLD)
                    {
                        ...
                        circle(next_copy, Point(cvRound(p2.x), cvRound(p2.y)), 5, Scalar(255,255,255), -1);
                        if(winnum == 4 || winnum == 5 || winnum == 6)
                            num_found++;
                    }
                    else if(isSideCam(winnum))
                    {
                        if(isLeftROI(p1, rect))
                        {
                            ...
                            else if(winnum == 4 || winnum == 5 || winnum == 6)
                            {
                                num_not_found++;
                                circle(next_copy, Point(cvRound(p2.x), cvRound(p2.y)), 5, Scalar(0,255,255), -1);
                                cout << "MAG_DIFF = " << abs(point_mag - median_mag_top) << "\tANG_DIFF = " << abs(point_ang - median_ang_top) << "\tSIDE" << endl;
                            }
                        }
                        ...
                        else
                        {
                            ...
                            else if(winnum == 4 || winnum == 5 || winnum == 6)
                            {
                                num_not_found++;
                                circle(next_copy, Point(cvRound(p2.x), cvRound(p2.y)), 5, Scalar(0,255,255), -1);
                                cout << "MAG_DIFF = " << abs(point_mag - median_mag_bot) << "\tANG_DIFF = " << abs(point_ang - median_ang_bot) << "\tSIDE" << endl;
                            }
                        }
                    } 
                    else    //front/rear camera or center ROI for left/right cam
                    {
                        if(isTopROI(p1, rect))
                        {
                            // Point is in a center ROI; larger magnitude difference, smaller angle difference
                            ...
                            else if(winnum == 4 || winnum == 5 || winnum == 6)
                            {
                                num_not_found++;
                                circle(next_copy, Point(cvRound(p2.x), cvRound(p2.y)), 5, Scalar(0,255,255), -1);
                                cout << "MAG_DIFF = " << abs(point_mag - median_mag_top) << "\tANG_DIFF = " << abs(point_ang - median_ang_top) << "\tCENTER" << endl;
                            }
                        }
                        else
                        {
                            ...
                            else if(winnum == 4 || winnum == 5 || winnum == 6)
                            {
                                num_not_found++;
                                circle(next_copy, Point(cvRound(p2.x), cvRound(p2.y)), 5, Scalar(0,255,255), -1);
                                cout << "MAG_DIFF = " << abs(point_mag - median_mag_bot) << "\tANG_DIFF = " << abs(point_ang - median_ang_bot) << "\tCENTER" << endl;
                            }
                        }
