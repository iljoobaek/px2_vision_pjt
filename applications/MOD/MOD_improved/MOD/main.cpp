...
int main( int argc, char** argv )
{
    ...
    //VideoCapture cap("data/forbes_pull_out_right.mp4");
    ...
    //if(savevideo) {
        ...
        outputVideo_seq.open("result/output_video.avi", CV_FOURCC('M','J','P','G'), cap.get(CV_CAP_PROP_FPS), S, true);
    ...
    while(true)
    {
        ...
        //showorb2(prv, featimg, feats);//Computing features on this images.
        
        vector<KeyPoint> feats;
        if(use_gray)
        {
            Rect Rect1 = Rect(0,0,640,360); 
            Rect Rect2 = Rect(640,0,640,360); 
            Rect Rect3 = Rect(0,360,640,360); 
            Rect Rect4 = Rect(640,360,640,360);
            Mat prv1 = prv(Rect1);
            Mat prv2 = prv(Rect2);
            Mat prv3 = prv(Rect3);
            Mat prv4 = prv(Rect4);
            vector<KeyPoint> feats1;
            vector<KeyPoint> feats2;
            vector<KeyPoint> feats3;
            vector<KeyPoint> feats4;

            
            showorb2(prv1, featimg, feats1, 0, 0);
            showorb2(prv2, featimg, feats2, 640, 0);
            showorb2(prv3, featimg, feats3, 0, 360);
            showorb2(prv4, featimg, feats4, 640, 360);
            //showfast(prv, featimg, feats);//Computing features on this images.

            feats.reserve(feats1.size() + feats2.size() + feats3.size() + feats4.size());
            feats.insert(feats.end(), feats1.begin(), feats1.end());
            feats.insert(feats.end(), feats2.begin(), feats2.end());
            feats.insert(feats.end(), feats3.begin(), feats3.end());
            feats.insert(feats.end(), feats4.begin(), feats4.end());
        }
        ...
        if (points[0].size() > 0) 
	    {
            ...
            calcOpticalFlowPyrLK(
                    prv, next, 
                    points[0], points[1],
                    status, err, Size(32,32), 3);//, TermCriteria(TermCriteria::COUNT+TermCriteria::EPS, 30, 0.01), 8, 0.01);
        ...
        prv = next.clone();
        PERF("\n");
        cout << "\nnum_found: " << num_found << "\tnum_total: " << num_total << "\tnum_found / num_total: " << num_found / num_total << "\tnum_not_found: " << num_not_found << "\t" << num_total - num_found << endl;
