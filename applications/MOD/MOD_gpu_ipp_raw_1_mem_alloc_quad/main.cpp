...
int main( int argc, char** argv )
{
    ...
    Mat topBuffer, botBuffer, allBuffer;
    hconcat(matrices, allBuffer);
    hconcat(matInputBuffer,matInputBuffer1,topBuffer);
    hconcat(matInputBuffer2,matInputBuffer3,botBuffer);
    vconcat(topBuffer,botBuffer, allBuffer);
    ...
    gpu::resize(prv_temp_full, prv_temp, Size(1280,720));
    //gpu::GpuMat prv_temp;
    ...
    while(true)
    {
        ...
        gpu::resize(next_temp_full, next_temp, Size(1280,720));
        if(next_temp.empty())
            break;
