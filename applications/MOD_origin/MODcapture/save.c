...
{   
    ...
    if (NvMediaImageLock(imgSrc, NVMEDIA_IMAGE_ACCESS_WRITE, &surfaceMap) !=
  	  ...
  	  srcHeight = surfaceMap.height; //printf("Height=%d",srcHeight);
    	  srcWidth  = surfaceMap.width; //printf("Width =%d",srcWidth);

    if (srcAttr[NVM_SURF_ATTR_SURF_TYPE].value == NVM_SURF_ATTR_SURF_TYPE_RAW) {
        srcPitch = srcWidth * rawBytesPerPixel; //printf("raw bytes %d",rawBytesPerPixel);
    ...
    if (!(pSrcBuff = (NvU8*)malloc(srcImageSize))) {
	...
	cv::Mat Image;cv::Mat inverted;
	//static int framecount=0;
	cv::Mat bayer16BitMat(srcHeight, srcWidth, CV_16UC1, pSrcBuff);
	//cv::Mat rgb(srcHeight,srcWidth,CV_16UC3);
	...
	bayer8BitMat.convertTo(bayer8BitMat, CV_8UC3, 0.015625);
	// Convert the Bayer data to 8-bit RGB
	//cv::cvtColor(bayer16BitMat,rgb,CV_BayerGR2RGB);
	//cv::Mat rgba(srcWidth,srcHeight,CV_8UC3);
	//rgb.convertTo(rgba,CV_8UC3,255);
	cv::Mat rgb8BitMat(srcHeight, srcWidth, CV_8UC1);
	cv::cvtColor(bayer8BitMat, rgb8BitMat, CV_BayerGR2RGB);
	resize(rgb8BitMat, Image,  cv::Size(1920,1208));
	//set(cv2.CAP_PROP_EXPOSURE, 40) 
	imshow( "\n imageName", Image); printf("\n Hello!");
	cvWaitKey(2);
	//free(imagedata);
	//framecount++; printf("\n framecount is %d",framecount);
    ...
    if (!(pDstBuff = (NvU8*)calloc(1, dstImageSize))) {
        LOG_ERR("%s: Out of memory\n", __func__);
    ...
    status = _ConvGetPixelOffsets((NvMediaRawPixelOrder)pixelOrder, xOffsets, yOffsets);
    if (status != NVMEDIA_STATUS_OK) {
    ...
    return (NvMediaStatus)srcHeight;
}
        ...
        if (threadCtx->displayEnabled) {
            ...
            if (attr[NVM_SURF_ATTR_SURF_TYPE].value == NVM_SURF_ATTR_SURF_TYPE_RAW) {
                ...
                status = _ConvRawToRgba(image,
                                        convertedImage,
                                        threadCtx->rawBytesPerPixel,
                                        threadCtx->pixelOrder); //printf("\n raw pixels %d",threadCtx->rawBytesPerPixel);printf("\n Pixel Order %d",threadCtx->pixelOrder);
             //   if (status != NVMEDIA_STATUS_OK) {
...
SaveInit(NvMainContext *mainCtx)
{
    ...
    saveCtx = (NvSaveContext*)mainCtx->ctxs[SAVE_ELEMENT];
    memset(saveCtx,0,sizeof(NvSaveContext));
    captureCtx = (NvCaptureContext*)mainCtx->ctxs[CAPTURE_ELEMENT];
   ...
   // getimage=image;
    ...
    for (i = 0; i < saveCtx->numVirtualChannels; i++) {
        ...
        if (saveCtx->threadCtx[i].conversionQueue) {
            while (IsSucceed(NvQueueGet(saveCtx->threadCtx[i].conversionQueue, &image, 0))) { getimage=image;
                if (image) {
...
SaveProc(NvMainContext *mainCtx)
{
    ...
    saveCtx = (NvSaveContext*)mainCtx->ctxs[SAVE_ELEMENT];
    compositeCtx = (NvCompositeContext*)mainCtx->ctxs[COMPOSITE_ELEMENT];
