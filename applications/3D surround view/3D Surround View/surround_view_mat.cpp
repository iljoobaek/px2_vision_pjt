...
// Include server.h to read from camera
#include <server.h>
...
GLuint ConvertIplToTexture(cv::Mat *image, int feed)
{
	GLuint texture;
	...
	cv::Mat *target_im = cvCreateImage(cvGetSize(image), image->depth, image->nChannels);
	//printf("%d\n", __LINE__);
...
GLuint ConvertIplToTexturePX2(cv::Mat *image)
{
	GLuint texture;
	int startX, startY, sizeX, sizeY;
